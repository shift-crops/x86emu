...
Emulator::Emulator(size_t size, uint16_t cs, uint16_t ip, const char *disk_name) : Hardware(size) {
	...
	set_memio(0xa0000, 0x10000, ui->get_display()->get_vram(MODE_GRAPHIC));
	set_memio(0xb0000, 0x8000,  ui->get_display()->get_vram(MODE_TEXT_MONO));
	set_memio(0xb8000, 0x8000,  ui->get_display()->get_vram(MODE_TEXT_COLOR));
